...
namespace Ophelia_Test_V21.AFuncionesGlobales.CRUDProgramas.ModulosNm_2
{
    class CrudKnmrardp
    {
        public static void KNmRardpCRUD(WindowsDriver<WindowsElement> desktopSession, List<string> variables, int flag) {
            ...
            List<int> fieldIndex = new List<int>() { 2, 8, 7, 6 };
            if (flag == 0)
            {
                ...
                desktopSession.Mouse.MouseMove(editFields[fieldIndex[fieldIndex.Count-1]].Coordinates);
                Thread.Sleep(1000);
                desktopSession.Mouse.Click(null);
                Thread.Sleep(1000);
                editFields[fieldIndex[fieldIndex.Count-1]].Clear();
                Thread.Sleep(1000);
