...
namespace Ophelia_Test_V21.AFuncionesGlobales.CRUDProgramas.ModulosNm_3
{
    class CrudKnmtipce
    {
        public static void KNmTipceCRUD(WindowsDriver<WindowsElement> desktopSession, List<string> variables, int flag) {
            ...
            if (flag == 0)
            {
                ...
                comboBoxes[0].Clear();
                Thread.Sleep(1000);
                comboBoxes[0].SendKeys(variables[2]);
                Thread.Sleep(1000);
        ...
        public static void KNmTipceCRUDDet(WindowsDriver<WindowsElement> desktopSession, List<string> variables, int flag) {
            var wait = new DefaultWait<WindowsDriver<WindowsElement>>(desktopSession)
            {
                Timeout = TimeSpan.FromSeconds(120),
                PollingInterval = TimeSpan.FromSeconds(1),
            ...
            if (flag == 0)
            {
                desktopSession.Mouse.MouseMove(grids[0].Coordinates, 20, grids[0].Size.Height / 7);
                Thread.Sleep(1000);
