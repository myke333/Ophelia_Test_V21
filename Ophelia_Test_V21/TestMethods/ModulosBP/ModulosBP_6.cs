...
{
    /// <summary>
    /// Descripción resumida de ModulosBP_6
    /// </summary>
    [TestClass]
    public class ModulosBP_6:FuncionesVitales
    {
        ...
        public void AbrirPrograma()
        {
            string motor = "SQL";
            //string motor = "ORA";
            AbrirPrograma a = new AbrirPrograma("KBpTipbe", "UCalida1");
            desktopSession = a.Start2(motor, "");
            //string file = CrearDocumentoWordDinamico("KFdCurso", "PruebasCrud", "SQL");

            Thread.Sleep(20000000);

            //AbrirPrograma a = new AbrirPrograma("KCoEncue", "ODESAR");
            //desktopSession = a.Start2(motor, "");

            //string file = CrearDocumentoWordDinamico("KCoEncue", "PruebasCrud", "SQL");
            //List<string> crudVariables = new List<string> { "40", "4", "93", "1", "G", "09/12/2020", "10/12/2020"};

            //string file = CrearDocumentoWordDinamico("KCoEncue", "PruebasCrud", "ORA");
            //List<string> crudVariables = new List<string> { "99", "13", "1", "A" };

            ////////CrudKnmpmupc.CRUD(desktopSession);
            ...
            ////ErrorValidacion = ValidacionesQueries.ValidacionCabecera(Tabla, user, motor, Campo, ID, Valor, "VAL_TOTA", c, ErrorValidacion, "No se edito correctamente", 1);
        ...
        public void KBpParamCheckList()
        {
            ...
            foreach (DataRow rowsi in OrderExecutionCase.Tables[0].Rows)
            {
                ...
                if (methodname.Replace(" ", string.Empty) == "Ophelia_Test_V21.TestMethods.ModulosBP.ModulosBP_6.KBpParamCheckList")
                {
...

                            {
                                ...
                                try
                                {
                                    ...
                                    if (leeprograma)
                                    {
                                        ...
                                        CrudKbpparam.AgregarCrud(desktopSession, 0, crudPrincipal);
                                        ...
                                        CrudKbpparam.AgregarCrud(desktopSession, 1, crudPrincipal);
                                        ...
                                        //QBE
                                        errors = FuncionesGlobales.QBEQry(desktopSession, TipoQbe, QbeFilter, file, camp);
                                        if (errors.Count > 0) { foreach (string er in errors) { ErrorValidacion.Add(er); } }
        ...
        public void KBpCatbeCheckList()
        {
            ...
            foreach (DataRow rowsi in OrderExecutionCase.Tables[0].Rows)
            {
                ...
                if (methodname.Replace(" ", string.Empty) == "Ophelia_Test_V21.TestMethods.ModulosBP.ModulosBP_6.KBpCatbeCheckList")
                {
                    DataSet sta = SqlAdapter.SelectOrderExecution("P", TableOrder, plans, suite, CaseId);
                    string endstatus = null;
                    foreach (DataRow rowsta in sta.Tables[0].Rows)
                    ...
                    if (endstatus == "True")
                    {
                        ...
                        foreach (DataRow rows in DataCase.Tables[0].Rows)
                        {
                                ...
                                )
                            {
                                ...
                                //List<string> crudDetalle = new List<string> { Registro, Requisito, Especificacion };
                                //CREAR EL DOCUMENTO DEL REPORTE DE LA PRUEBA
                                string file = CrearDocumentoWordDinamico(codProgram, "BP", motor);
                                try
                                {
                                    ...
                                    if (leeprograma)
                                    {
                                        ...
                                        string pdf1 = @"C:\Reportes\ReportePDF1_" + "Preliminar" + "_" + Hora();
                                        errors = FuncionesGlobales.ReportePreliminar(desktopSession, BanderaPreli, file, pdf1);
                                        //CrudKfdcurso.Preliminar(desktopSession, file, pdf1, BanderaPreli);
                                        if (errors.Count > 0) { foreach (string er in errors) { ErrorValidacion.Add(er); } }
                                        //Rectificar Pie Pagina PDF PRELIMINAR
                                        listaResultado = Textopdf(pdf1, codProgram, user);
                                        listaResultado.ForEach(e => ErrorValidacion.Add(e));
        ...
        public void KBpTipevCheckList()
        {
            ...
            foreach (DataRow rowsi in OrderExecutionCase.Tables[0].Rows)
            {
                ...
                if (methodname.Replace(" ", string.Empty) == "Ophelia_Test_V21.TestMethods.ModulosBP.ModulosBP_6.KBpTipevCheckList")
                {
                    DataSet sta = SqlAdapter.SelectOrderExecution("P", TableOrder, plans, suite, CaseId);
                    string endstatus = null;
                    foreach (DataRow rowsta in sta.Tables[0].Rows)
                    ...
                    if (endstatus == "True")
                    {
                        ...
                        foreach (DataRow rows in DataCase.Tables[0].Rows)
                        {
...

                            {
                                ...
                                //LISTA CRUD PRINCIPAL
                                List<string> crudPrincipal = new List<string>() { Codigo, Nombre, EditarNombre };
                                ...
                                try
                                {
                                    ...
                                    if (leeprograma)
                                    {
                                        ...
                                        CrudKbptipev.AgregarCrud(desktopSession, 0, crudPrincipal);
                                        newFunctions_4.ScreenshotNuevo("Insertar Datos", true, file);
                                        ...
                                        CrudKbptipev.AgregarCrud(desktopSession, 1, crudPrincipal);
                                        newFunctions_4.ScreenshotNuevo("Datos Editados", true, file);
