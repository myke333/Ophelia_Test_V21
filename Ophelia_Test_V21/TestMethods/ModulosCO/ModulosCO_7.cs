...
namespace Ophelia_Test_V21.TestMethods.ModulosBI
{
    ...
    /// </summary>
    [TestClass]
    public class ModulosCO_7 : FuncionesVitales
    {
        ...
        public void KCoMonitChecKlist()
        {
            ...
            foreach (DataRow rowsi in OrderExecutionCase.Tables[0].Rows)
            {
                ...
                if (methodname.Replace(" ", string.Empty) == "Ophelia_Test_V21.TestMethods.ModulosBI.ModulosCO_7.KCoMonitChecKlist")
                {
...

                            {
                                ...
                                try
                                {
                                    ...
                                    if (leeprograma)
                                    {
                                        ...
                                        //AGREGAR PARAMETRO
                                        CrudKcomonit.KCoMonitCRUD(desktopSession, crudVariablesTEdit, "TEdit", 0, 0, "");

                                        newFunctions_4.ScreenshotNuevo("Se agrega la fecha", true, file);

                                        //Click en parciales
                                        CrudKcomonit.Click(desktopSession, 0);
                                        newFunctions_4.ScreenshotNuevo("Se da click en parciales", true, file);

                                        //QBE
                                        errors = FuncionesGlobales.QBEQry(desktopSession, TipoQbe, QbeFilter, file);
                                        if (errors.Count > 0) { foreach (string er in errors) { ErrorValidacion.Add(er); } }
                                        Thread.Sleep(1000);
                                        newFunctions_4.ScreenshotNuevo("Se hace el QBE", true, file);

                                        /////BOTON ACEPTAR
                                        CrudKcomonit.ClickButtonAceptar(desktopSession);
                                        newFunctions_4.ScreenshotNuevo("Se da click en el boton aceptar", true, file);
